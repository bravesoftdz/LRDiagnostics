...
type
  ...
  protected
    ...
    procedure FromJsonString(AValue: String);
  public
    constructor Create(AExportRequest: TExportRequest = nil);
    ...
    property AsJson: String read ToJsonString write FromJsonString;
    class function GetFromJsonString(AValue: String): TExportRequest;
  end;
...
{$REGION 'TExportRequest'}
constructor TExportRequest.Create(AExportRequest: TExportRequest = nil);
var
  i: Integer;
begin
  FMaxAgeHours := 0;
  ...
  if nil <> AExportRequest then
  begin
    for i := 0 to (AExportRequest.Components.Count - 1) do
      FComponents.Add(TLRComponent.Create(AExportRequest.Components[i]));
  end;
end;

destructor TExportRequest.Destroy;
begin
  FComponents.Free;
  inherited Destroy;
end;

function TExportRequest.ToJsonString: String;
var
  LObj: TJsonObject;
begin
  LObj := ToJSONObject;
...
procedure TExportRequest.FromJsonString(AValue: String);
var
  LObj: TJsonObject;
begin
  LObj := TJsonObject.ParseJSONValue(AValue) As TJsonObject;
...
function TExportRequest.ToJsonObject: TJsonObject;
var
...
procedure TExportRequest.FromJsonObject(AValue: TJsonObject);
var
  LComponents: TJsonArray;
...
class function TExportRequest.GetFromJsonString(AValue: String): TExportRequest;
begin
  Result := TExportRequest.Create;
  Result.AsJson := AValue;
end;
{$ENDREGION}
